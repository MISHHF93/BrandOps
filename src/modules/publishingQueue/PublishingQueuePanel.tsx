...
const statusPillClass: Record<QueueStatus, string> = {
  ...
  'due-soon': 'bg-amber-500/20 text-amber-200 border border-amber-500/40',
  'ready-to-post': 'bg-emerald-500/20 text-emerald-200 border border-emerald-500/40',
  posted: 'bg-indigo-500/20 text-indigo-200 border border-indigo-500/40',
  skipped: 'bg-rose-500/20 text-rose-200 border border-rose-500/40'
};
...
export function PublishingQueuePanel() {
  ...
  return (
    <section className="bo-card space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Professional scheduling workflow for solo LinkedIn publishing (manual posting only).
          </p>
        </div>
      ...
      <article className="rounded-xl border border-slate-800 bg-slate-950/45 p-3 space-y-2">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          ...
          <textarea
            ...
            rows={4}
            placeholder="Paste LinkedIn content"
      ...
      {(viewMode === 'list' ? queueItems : Object.values(calendarGroups).flat()).map((item) => (
        <article key={item.id} className="rounded-xl border border-slate-800 bg-slate-950/40 p-3 text-sm">
          ...
          {item.overdue ? <p className="mt-2 text-xs text-rose-300">Overdue: past due and not posted.</p> : null}
          {item.checklist ? (
            <p className="mt-2 whitespace-pre-wrap text-xs text-slate-300">Checklist:\n{item.checklist}</p>
          ) : null}
          ...
          <div className="mt-3 grid gap-2 md:grid-cols-3">
            <input
              ...
              className="rounded-lg border border-slate-700 bg-slate-950/55 px-2 py-1 text-xs"
            />
            <select
      ...
      {viewMode === 'calendar' ? (
        <div className="space-y-2">
          {Object.entries(calendarGroups).map(([day, items]) => (
            <div key={day} className="rounded-xl border border-slate-800 bg-slate-950/30 p-3">
              <p className="text-xs uppercase tracking-[0.14em] text-slate-400">{day}</p>

...
} from '../../shared/help/knowledgeCenterTopics';
    ...
    <>
      ...
      >
        ...
        <div className="mx-auto max-w-5xl space-y-6">
          <section aria-labelledby="kc-daily-heading" className="bo-card space-y-4 border border-primary/30 bg-bg/70 p-4 md:p-5">
          ...
          <section aria-labelledby="kc-reference-heading" className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-textMuted">
                  Jump to a topic or expand for the full guide.
                </p>
              </div>
            </div>
            ...
            <nav aria-label="Knowledge Center topics" className="flex flex-wrap gap-x-3 gap-y-1.5 text-xs">
              {knowledgeCenterTopics.map((topic) => (
                <a
                  key={topic.id}
                  className="bo-link !py-0.5"
                  href={`?${QUERY.helpTopic}=${encodeURIComponent(topic.id)}`}
                >
                  {topic.title}
                </a>
              ))}

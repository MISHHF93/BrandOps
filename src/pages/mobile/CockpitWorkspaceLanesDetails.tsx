...
export const CockpitWorkspaceLanesDetails = ({
  ...
  <details className="group scroll-mt-28 rounded-xl border border-border/40 bg-bgSubtle/50 p-3 open:shadow-inner">
    <summary
      className={`cursor-pointer list-none text-xs font-semibold uppercase tracking-wide text-textMuted ${btnFocus} [&::-webkit-details-marker]:hidden`}
    >
    ...
    <ul className="mt-3 space-y-3">
      {workspaceModules
        ...
        .map((m) => {
          ...
          return (
            <li key={m.id} className="rounded-lg border border-border/30 bg-surface/55 p-2.5">
              <p className="text-[12px] font-medium text-text">{m.title}</p>
              <p className="mt-0.5 text-[10px] leading-snug text-textMuted">{m.description}</p>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {section ? (
                  <button
                    type="button"
                    onClick={() => onSelectWorkstream(section)}
                    className={`rounded-full border border-borderStrong/50 bg-surface/70 px-2 py-1 text-[10px] text-text ${btnFocus}`}
                  >
                ...
                {seed ? (
                  <button
                    ...
                    className={`rounded-full border border-borderStrong/50 bg-surface/70 px-2 py-1 text-[10px] text-text ${btnFocus} disabled:cursor-not-allowed disabled:opacity-50`}
                  >
                    Run starter in Chat
                  </button>
                ) : null}

...
export function computeProjectIntelligence(input: ProjectIntelligenceInput): ProjectIntelligence {
  const project = (input.state.projects ?? []).find((p) => p.id === input.projectId);
  ...
  const verifiedEvents = relevantEvents.filter((e) =>
  ...
  const avgConfidence = verifiedEvents.length > 0
    ? verifiedEvents.reduce((sum, e) => sum + e.confidence, 0) / verifiedEvents.length
    : 0;
  const professionalValue = Math.min(1, (avgConfidence * 0.7 + Math.min(1, totalEvidence / 10) * 0.3));

  // Missing documentation
  const missingDocumentation: string[] = [];
  if (verifiedEvents.length > 0 && !project.artifactIds.some((id) =>
  ...
  const hasContentOpportunity = verifiedEvents.some((e) =>
    ['feature-shipped', 'repository-released', 'product-launched', 'documentation-published', 'open-source-contribution'].includes(e.kind)
  );
  const contentPotential = hasContentOpportunity ? Math.min(1, 0.5 + avgConfidence * 0.3 + Math.min(1, totalEvidence / 15) * 0.2) : 0.1;
  ...
  // Evidence summary
  const evidenceSummary = `Project "${project.name}" has ${verifiedEvents.length} verified achievement(s)` +
  ...
  if (verifiedEvents.some((e) => e.kind === 'project-milestone-reached' && e.verificationStatus === 'USER_VERIFIED')) {
...
export function deriveProjectStatus(achievements: AchievementCandidate[], plans: Plan[]): ProjectStatus {
  // Use event-based checking since AchievementCandidate doesn't have verificationStatus
  const verifiedMilestones = achievements.some((a) =>
    a.kind === 'project-milestone-reached'
  );
  const hasActivePlans = plans.some((p) =>

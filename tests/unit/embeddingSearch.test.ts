...
import {
  cosineSimilarity,
  queryEmbeddingIndex,
  searchContentByRelevance
} from '../../src/services/ai/embeddingSearch';
import type { BrandOpsData } from '../../src/types/domain';

function makeWorkspace(
  embeddingEntries: BrandOpsData['embeddingIndex']['entries'] = [],
  contentLibrary: BrandOpsData['contentLibrary'] = []
): BrandOpsData {
  return {
    modules: [],
  ...
  it('respects topK limit', () => {
    const ws = makeWorkspace(
      ...
      Array.from({ length: 20 }, (_, i) => ({
        id: `item-${i}`,
        type: 'post-draft' as const,
        status: 'draft' as const,
        title: `AI Engineering Topic ${i}`,
        body: `Building AI systems number ${i}`,
        tags: [],
        createdAt: '2026-01-01',
        updatedAt: '2026-01-01'
      }))

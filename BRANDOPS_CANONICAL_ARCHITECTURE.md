...
│   │   │                     convertToPlan, events, gateway, idempotency, MCP, proposals,
│   │   │                     sessions, trust boundaries, validation
...
└── tests/                   # 152 test files, 982 tests (updated 2026-08-31)
```
...
### Gateway (gateway.ts)
Auth → Authorize → Injection screen → Idempotency → Dispatch → Audit + Checkpoint + Trace
...
### MCP
10 capabilities, 1:1 tool mapping, input schemas defined.
Stdio transport exists (mcp/server.ts, scripts/mcp-gateway.mjs).
Token↔workspace sync: manual export only ("Export workspace for MCP" in Connected Agents panel).

### Agent Identity Registry
**STATUS: PARTIAL** — Sessions + gateway exist. No registry with trust levels:
READ_ONLY, CONTEXT_CONSUMER, PROPOSER, ACTION_REQUESTER.
...
## 8. Policy Engine (policyEngine.ts)

**STATUS: ABSENT (corrected 2026-08-31).** The `policyEngine.ts` module documented here with 11 policy decisions (CAN_READ_CONTEXT, CAN_CREATE_DRAFT, CAN_PROPOSE_TWIN_CHANGE, CAN_CREATE_PLAN, CAN_REQUEST_EXTERNAL_ACTION, REQUIRES_APPROVAL, REQUIRES_SENSITIVE_CONFIRMATION, CAN_INGEST_ACTIVITY, CAN_VERIFY_ACHIEVEMENT, CAN_PROMOTE_TO_TOWN, CAN_EXECUTE_PLAN) **does not exist in the source tree** — it was a phantom/never-implemented module (the earlier `controlPlane/` tree was also deleted as dead code). These policy concerns are instead implemented decentralically:
...
2. **Cross-workspace isolation** — No test proving workspace A cannot access workspace B data.
   Local-first architecture makes this low-risk, but untested.
3. **Approval bypass** — No test proving an approved plan cannot be executed without an
   approval checkpoint, or that rejected plans cannot be re-executed.
